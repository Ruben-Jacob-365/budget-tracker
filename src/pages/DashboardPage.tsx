      ...
      {accounts.length > 0 && (
        <section aria-labelledby="accounts-heading">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {accounts.map((acc) => (
              <AccountBalanceCard key={acc.id} account={acc} fmt={fmt} />
            ))}
          </div>
        </section>
      )}
        ...
        {loading ? (
          ...
          <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 overflow-hidden">
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {recent.map((tx) => (
                <RecentRow key={tx.id} tx={tx} catMap={catMap} fmt={fmt} />
              ))}
            </div>
          </div>
        )}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-lg shrink-0 ml-2" aria-hidden="true">
          {isCredit ? "💳" : "🏦"}
        </span>
      </div>
...
function RecentRow({
  ...
  tx: Transaction;
  catMap: Map<string, { icon?: string; name: string; color?: string }>;
  fmt: (n: number) => string;
}) {
    ...
    <div className="flex items-center gap-3 px-4 py-3.5">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-slate-900 dark:text-white truncate">
          {tx.merchant || cat?.name || "Unknown"}
        </p>
        <p className="text-xs text-slate-500 dark:text-slate-400">{tx.date}</p>
      </div>
      <span
        className={`text-sm font-semibold shrink-0 ${isIncome ? "text-emerald-600 dark:text-emerald-400" : "text-slate-900 dark:text-white"}`}
      >
        {isIncome ? "+" : "-"}
        {fmt(tx.amount)}
      </span>
